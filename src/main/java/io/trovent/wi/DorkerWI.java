...
public class DorkerWI implements WorkItemHandler {
	...
	public void executeWorkItem(WorkItem workItem, WorkItemManager manager) {
		...
		boolean dorker_run = (boolean) workItem.getParameter("dork_run");
		...
		if (dorker_run) {
			try {
				...
				b.param(1, domain);
				File file = new File(Properties.workitems_dir + "/dork_exec.py");
				result = ex.execute(true, file.getAbsolutePath(),"-d",domain);
				b.result(result);
			} catch (Exception e) {
